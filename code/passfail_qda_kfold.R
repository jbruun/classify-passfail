# Quadratic discriminant analysis on pass/fail centrality with k-fold cross-validation.
# Last modified: 1/7/21 (forked from passfail_lda_kfold.R)
# 
# Status: Getting rank deficiency errors for predCD, predJustCD, and predJustICS. 
...
predPS <- kfoldQDA(centPS, k = kf, predictors = preds)
predCD <- kfoldQDA(centCD, k = kf, predictors = preds)
predICS <- kfoldQDA(centICS, k = kf, predictors = preds)

# Predict just-pass/just-fail (2/0)
predJustPS <- kfoldQDA(centPS, outcome = "justpass", k = kf, predictors = preds)
predJustCD <- kfoldQDA(centCD, outcome = "justpass", k = kf, predictors = preds)
predJustICS <- kfoldQDA(centICS, outcome = "justpass", k = kf, predictors = preds)
