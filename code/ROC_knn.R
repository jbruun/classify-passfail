...
ROC_PS_knn<-list()
for(i in 1:100){
  predPS_x<-jackPredKNN(centPS,nK=i)
...
ROC_CD_knn<-list()
for(i in 1:100){
  predCD_x<-jackPredKNN(centCD,nK=i)
...
ROC_ICS_knn<-list()
for(i in 1:100){
  predICS_x<-jackPredKNN(centICS,nK=i)
...
for(i in 1:100){
  predPS_x<-jackPredKNN(centPS,outcome = "justpass",nK=i)
  ROC<-ROCplusWeeks(predPS_x$allpred)
...
for(i in 1:100){
  predCD_x<-jackPredKNN(centCD, outcome = "justpass",nK=i)
  ROC<-ROCplusWeeks(predCD_x$allpred)
...
for(i in 1:100){
  predICS_x<-jackPredKNN(centICS, outcome = "justpass",nK=i)
  ROC<-ROCplusWeeks(predICS_x$allpred)
