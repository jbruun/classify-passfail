...
######PR-TE-H Models######
pred_log_FGC<-jackPredLog(centPS,predictors = c("fci_pre_c","gender","cohort"))
pred_log_FG<-jackPredLog(centPS,predictors = c("fci_pre_c","gender"))
pred_log_GC<-jackPredLog(centPS,predictors = c("gender","cohort"))
pred_log_FC<-jackPredLog(centPS,predictors = c("fci_pre_c","cohort"))
pred_log_F<-jackPredLog(centPS,predictors = c("fci_pre_c"))
pred_log_G<-jackPredLog(centPS,predictors = c("gender"))
pred_log_C<-jackPredLog(centPS,predictors = c("cohort"))


roc_log<-list()
roc_log[[1]]<-roc(pred_log_FGC$pass,as.numeric(pred_log_FGC$Week1),auc=T,ci=T)
roc_log[[2]]<-roc(pred_log_FG$pass,as.numeric(pred_log_FG$Week1),auc=T,ci=T)
roc_log[[3]]<-roc(pred_log_GC$pass,as.numeric(pred_log_GC$Week1),auc=T,ci=T)
roc_log[[4]]<-roc(pred_log_FC$pass,as.numeric(pred_log_FC$Week1),auc=T,ci=T)
roc_log[[5]]<-roc(pred_log_F$pass,as.numeric(pred_log_F$Week1),auc=T,ci=T)
roc_log[[6]]<-roc(pred_log_G$pass,as.numeric(pred_log_G$Week1),auc=T,ci=T)
roc_log[[7]]<-roc(pred_log_C$pass,as.numeric(pred_log_C$Week1),auc=T,ci=T)
log_auc<-c(rocPS_log_PTH[[1]]$auc,rocPS_log_PTH[[2]]$auc,rocPS_log_PTH[[3]]$auc,rocPS_log_PTH[[4]]$auc,
                  rocPS_log_PTH[[5]]$auc,rocPS_log_PTH[[6]]$auc,rocPS_log_PTH[[7]]$auc)
log_ciL<-c(rocPS_log_PTH[[1]]$ci[1],rocPS_log_PTH[[2]]$ci[1],rocPS_log_PTH[[3]]$ci[1],rocPS_log_PTH[[4]]$ci[1],
                  rocPS_log_PTH[[5]]$ci[1],rocPS_log_PTH[[6]]$ci[1],rocPS_log_PTH[[7]]$ci[1])
log_ciH<-c(rocPS_log_PTH[[1]]$ci[3],rocPS_log_PTH[[2]]$ci[3],rocPS_log_PTH[[3]]$ci[3],rocPS_log_PTH[[4]]$ci[3],
...
dev.off()
pdf(file = "plots/ROC_AUC_plots/ROC01_lda_jpf.pdf",   # The directory you want to save the file in
    width = 8.5, # The width of the plot in inches
    height = 14) # The height of the plot in inches
par(mfrow=c(4,2))
