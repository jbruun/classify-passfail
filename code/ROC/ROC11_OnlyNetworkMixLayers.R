...
for(i in 1:100){
  predE_x<-jackPredLog(centAll,predictors = c("ICS_PageRank","CD_PageRank"),p=i/100)
...
for(i in 1:100){
  predE_x<-jackPredLDA(centAll,predictors = c("ICS_PageRank","CD_PageRank"),p=i/100)
  ROC<-ROCplusWeeks(predE_x)
  ROC_E_lda[[i]]<-ROC
}
...
save(ROC_A_knn,ROC_B_knn,ROC_C_knn,ROC_D_knn,
     ...
     file="data/ROC_AUC/ROC11_NB_knn.Rdata")
